...
// (recipient, amount) decoded from the proven public_inputs.
...
pub fn enforce_sibling_policy(
    ...
    require_keys_eq!(*ix_sysvar.key, SYSVAR_INSTRUCTIONS_ID, GatewayError::InvalidVk);
        ...
        match dispatch(&ix)? {
            ...
            Outcome::Extracted(action) => {
                require_keys_eq!(
                    action.recipient,
                    expected_recipient,
                    GatewayError::PolicyRecipientMismatch
                );
    ...
    require!(transfer_count > 0, GatewayError::SiblingMissing);
    require_eq!(total_amount, expected_amount, GatewayError::PolicyAmountMismatch);

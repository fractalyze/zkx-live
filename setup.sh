...
echo
echo "  2. Compile + zkey for the demo circuit (V5):"
echo "       cd circuits"
echo "       circom pay_with_self_attest.circom --r1cs --c -l node_modules -o build/"
echo "       ./node_modules/.bin/snarkjs groth16 setup build/pay_with_self_attest.r1cs ptau/pot22_hez.ptau build/pay_with_self_attest_0000.zkey"
echo "       ./node_modules/.bin/snarkjs zkey contribute build/pay_with_self_attest_0000.zkey build/pay_with_self_attest_final.zkey -e='snap'"
echo "       ./node_modules/.bin/snarkjs zkey export verificationkey build/pay_with_self_attest_final.zkey build/pay_with_self_attest_vk.json"
echo "       ( cd build/pay_with_self_attest_cpp && make )"
echo
echo "  3. Start the prover service (warm GPU, ~3 s startup):"
echo "       PROVER_ZKEY=\$PWD/circuits/build/pay_with_self_attest_final.zkey \\"

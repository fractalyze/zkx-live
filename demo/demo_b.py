...
def _run(validator: lib.Validator) -> int:
    ...
    print(f"[demo-b] payer: {payer.pubkey()}")
    ...
    # --- V2 proof: build_v2_input.mjs binds dst pubkey + signs Reclaim claim ---
    input_json = BUILD / "input.json"
    wtns = BUILD / "witness.wtns"
    proof_path = BUILD / "proof.json"
    pub_path = BUILD / "public.json"

    print("[demo-b] building V2 input bound to claimer pubkey ...")
    run(
        [
            "node",
            "build_v2_input.mjs",
    ...
    print("[demo-b] computing witness (V2 ~ 2.5 min on first run) ...")
    lib.gen_witness(
        CIRCUITS / "build" / "pay_with_reclaim_js" / "pay_with_reclaim.wasm",
        input_json,
        wtns,
    )
    print("[demo-b] generating proof via rabbit-py (V2 ~ 8-10 s) ...")
    lib.gen_proof(
        CIRCUITS / "build" / "pay_with_reclaim_final.zkey",
        wtns,
        proof_path,
        pub_path,
    )
    ...
    # --- VK upload (schema 1) ---
    vk_json = json.loads((CIRCUITS / "build" / "pay_with_reclaim_vk.json").read_text())
    ...
    tag = b"\xcd" * 32
    # V2 blob (1408 B) is too big for a single stage_proof tx — chunk it.
    ...
    for _ in range(60):
        try:
            ...
            pass
        time.sleep(0.5)
    print(f"[demo-b] claimer balance: {final_bal} (expected {AMOUNT})")
    if int(final_bal) != AMOUNT:
        ...
        return 1
    print(
        "[demo-b] PASS — same gateway, same verifier program, "
